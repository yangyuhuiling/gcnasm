...
__device__ inline void copy_local_a_to_peer(opus_a2a_gemm_kargs kargs, int comm_slot, int comm_slots) {
    ...
    for (int step = 1; step <= step_count; ++step) {
        ...
        for (size_t i = static_cast<size_t>(comm_slot * T::BLOCK_SIZE + tid);
             i < vec_count;
             i += stride * 8) {
            const size_t i0 = i;
        ...
        if (tid == 0) {
            ...
            __atomic_fetch_add(peer_ready + src_rank, 1u, __ATOMIC_RELAXED);
            asm volatile("s_waitcnt vmcnt(0)" ::: "memory");
            __builtin_amdgcn_fence(__ATOMIC_SEQ_CST, "");
        }
    }
...
template<typename T>
__device__ inline void wait_input_ready(opus_a2a_gemm_kargs kargs, int k_part, int tid, int wave_id) {
    if (tid == 0 && k_part != kargs.my_rank) {
        unsigned int* local_ready = static_cast<unsigned int*>(kargs.ready_local);
        while (flat_load_u32(local_ready + k_part) < static_cast<unsigned int>(kargs.comm_wgs)) {
        }
    } else {
...
#endif
    if (is_comm_wg) {
        copy_local_a_to_peer<T>(kargs, bx >> 3, kargs.comm_wgs);
        return;
    }
